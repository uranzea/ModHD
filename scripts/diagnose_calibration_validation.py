...
from tank_model.metrics import bias_pct, kge, nse, plot_error_metrics_heatmap, rmse
...
if "Qobs_m3s" not in df.columns:
    df["Qobs_m3s"] = np.nan
print(f"Archivo encontrado: {DEFAULT_FORCING_PATH.name}")
print(f"Ruta de entrada: {DEFAULT_FORCING_PATH}")
...
med_dt_h = infer_dt_hours(df.index)
med_dt_s = med_dt_h * 3600.0
...
report_cfg = ModelConfig(
    ...
    route=True,
    debug_balance=True,
    debug_csv_path=str(RESULTS_DIR / "diag_balance_desk_check.csv"),
)
print(report_cfg)
...
for it in range(1, n_iter+1):
    ...
    m = TankModel(params=p, config=calibration_cfg)
...
m_best = TankModel(params=best_params, config=report_cfg)
...
# ---------------------------------------------------------------------
out_csv = RESULTS_DIR / "desk_check_results.csv"
results.to_csv(out_csv, index_label="date")
...
sum_path = RESULTS_DIR / "desk_check_metrics_summary.csv"
...
ts_path = RESULTS_DIR / "desk_check_timeseries.png"
...
fdc_path = RESULTS_DIR / "desk_check_flow_duration_curves.png"
save_figure(fig_fdc, fdc_path)
...
scatter_path = RESULTS_DIR / "desk_check_scatter_plots.png"
...
ax = plot_error_metrics_heatmap(results["Q_obs_m3s"], results["Q_sim_m3s"])
heatmap_path = RESULTS_DIR / "desk_check_metrics_heatmap.png"
save_figure(ax.figure, heatmap_path)
